    ...
    while(stack.length > 0) {
      breakable {
        if(stack(0)("node") == null) {
          ...
          stack.remove(0)
          var currentNode: BinaryTree = nodeInfo("node").asInstanceOf[BinaryTree]
          var currentDepth: Int = nodeInfo("depth").asInstanceOf[Int]
          result += currentDepth
