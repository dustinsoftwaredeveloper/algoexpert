...
class JavaSolutionTwo {
  ...
  public static BST minHeightBst(List<Integer> inputArr) {
    return minHeightBstHelper(inputArr);
  } 
  public static BST minHeightBstHelper(List<Integer> inputArr) {
    return minHeightBstHelper(inputArr, null, 0, inputArr.size() - 1);
  }
  public static BST minHeightBstHelper(List<Integer> inputArr, BST root, int startIdx, int endIdx) {
    ...
    if (current == null) {
      ...
      if (inputArr.get(midIdx) < current.value) {
        current.left = new BST(inputArr.get(midIdx));
        current = current.left; 
      } else {
        current.right = new BST(inputArr.get(midIdx));
        current = current.right;
      }    
    }
