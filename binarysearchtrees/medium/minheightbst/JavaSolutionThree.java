...
class JavaSolutionThree {
  ...
  public static BST minHeightBstHelper(List<Integer> inputArr, int startIdx, int endIdx) {
    ...
    BST current = new BST(inputArr.get(midIdx));
    current.left = minHeightBstHelper(inputArr, startIdx, midIdx - 1);
    current.right = minHeightBstHelper(inputArr, midIdx + 1, endIdx);
    return current;
  }
}
